...
void GL_Set2D (void)
{
	...
	if (render_warp)
	{
		...
		const uint32_t screen_size[2] = { vr.width, vr.height };
		const float aspect_ratio_time[2] = { (float)vr.width / (float)vr.height, cl.time };
		...
		vkCmdDispatch(vulkan_globals.command_buffer, (vr.width + 7) / 8, (vr.height + 7) / 8, 1);
